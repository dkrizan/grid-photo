...
export default function App() {
  ...
  async function download() {
    if (files.length < groupSize) {
      setNotice({ type: 'warning', text: `Please add at least ${groupSize} photo${groupSize === 1 ? '' : 's'} before exporting.` })
      return
    }
    if (files.length % groupSize !== 0) {
      const missing = groupSize - (files.length % groupSize)
      setNotice({ type: 'warning', text: `Add ${missing} more photo${missing === 1 ? '' : 's'} to complete the next grid.` })
      return
    }
    setBusy(true)
    try {
      ...
      const aEl = document.createElement('a')
      aEl.href = url; aEl.download = 'photo-grids.zip'
        ...
        <header className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-2xl space-y-4">
            ...
            <p className="text-sm text-slate-300 sm:text-base">
              Arrange photo grids in seconds. Upload your shots, adjust layout, and export high-resolution sheets without leaving the browser.
            </p>
          </div>
        </header>
            ...
            <Flex direction="column" gap="4">
              ...
              >
                ...
                <Button variant="soft" color="violet" asChild>
                  <label htmlFor="file-input" className="flex cursor-pointer items-center gap-2 text-base font-medium">
                    <UploadIcon />
                    Browse images
                  </label>
                </Button>
                <Text size="2" className="text-slate-300">…or drag & drop files here</Text>
                <Text size="1" className="rounded-full border border-slate-700/60 bg-slate-900/60 px-3 py-1 text-slate-400">
              ...
              {(notice || !readyForDownload || !readyForPreview) && (
                <Flex direction="column" gap="2">
                  {notice && (
                    <Callout.Root color={notice.type === 'error' ? 'red' : notice.type === 'warning' ? 'amber' : notice.type === 'success' ? 'green' : 'blue'}>
                      <Callout.Icon>
                        <ImageIcon />
                      </Callout.Icon>
...
function PreviewCanvas({ group, opts, ready }: { group?: ImgFile[]; opts: ComposeOptions; ready: boolean }) {
  ...
  }, [group, opts.cols, opts.cropToFill, opts.dpi, opts.gapPx, opts.heightCm, opts.rotate90, opts.rows, opts.separatorColor, opts.widthCm, ready])
