...
function dims(img: { width: number; height: number }, rotate: boolean) {
  ...
  return rotate ? { w: height, h: width } : { w: width, h: height };
}
...
function drawImageRotated(
  ...
  if (!rotate) {
    // @ts-expect-error width/height exist at runtime
...
export function createGridCanvas(images: HTMLImageElement[], opts: ComposeOptions): HTMLCanvasElement {
...
export async function composeGrid(images: HTMLImageElement[], opts: ComposeOptions): Promise<Blob> {
